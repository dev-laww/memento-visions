...
public partial class CraftingManager : Node
{
    ...
    public override void _Ready()
    {
        ...
        var recipesData = JsonLoader.Load<List<Recipe>>(Constants.RECIPES_DATA_PATH);
        ...
        var craftableItems = items.Where(
            data => recipesData.Any(recipe => recipe.Result.UniqueName == data.UniqueName)
        );

        var resources = craftableItems.Select(item => GD.Load<Item>(item.Resource));

        recipes.AddRange(recipesData);
    ...
    public static Item Craft(Item item) => Instance.CraftItem(item);
}